...
##################################################################
# This is the start-up script for the Front Race Your Code robot #
#                                                                #
# It is responsible for managing the threads which control the   #
# rearward facing MonsterBorg during a race                      #
##################################################################
...
def MonsterMotors(driveLeft, driveRight):
	global TB
	TB.SetMotor1(-driveLeft  * Settings.maxPower) # Left motors (in reverse)
	TB.SetMotor2(-driveRight * Settings.maxPower) # Right motors (in reverse)
	SendUpdates()

# Send out updates over the network
def SendUpdates():
...
def ShowSettings():
	...
	print '[PID values]'
	print '    P0: %f	I0:%f	D0: %f' % (Settings.reverseKp0, Settings.reverseKi0, Settings.reverseKd0)
	print '    P1: %f	I1:%f	D1: %f' % (Settings.reverseKp1, Settings.reverseKi1, Settings.reverseKd1)
	print '    P2: %f	I2:%f	D2: %f' % (Settings.reverseKp2, Settings.reverseKi2, Settings.reverseKd2)
	print
