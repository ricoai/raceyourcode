...
#######################################################################
# This is the start-up script for the Front Race Your Code simulation #
#                                                                     #
# It is responsible for managing the threads which simulate the       #
...
def MonsterMotors(driveLeft, driveRight):
	global simLeft
	global simRight
	simLeft = -driveRight * Settings.simulationDrivePower
	simRight = -driveLeft * Settings.simulationDrivePower
		...
		while Globals.running:
			...
			if processor:
				...
				if frame is None:
					...
					print '!!! BAD IMAGE !!!'
					with Globals.frameLock:
						processor = Globals.processorPool.insert(0, processor)
					continue
				if okay:
