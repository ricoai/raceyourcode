...
class ControlLoop(threading.Thread):
	...
	def __Reset__(self):
		...
		if Globals.frontRobot:
			self.Kp0 = Settings.forwardKp0
			self.Ki0 = Settings.forwardKi0
			self.Kd0 = Settings.forwardKd0
			self.Kp1 = Settings.forwardKp1
			self.Ki1 = Settings.forwardKi1
			self.Kd1 = Settings.forwardKd1
			self.Kp2 = Settings.forwardKp2
			self.Ki2 = Settings.forwardKi2
			self.Kd2 = Settings.forwardKd2
			self.clipMax = Settings.forwardClipI
			self.clipMin = -Settings.forwardClipI
		else:
			self.Kp0 = Settings.reverseKp0
			self.Ki0 = Settings.reverseKi0
			self.Kd0 = Settings.reverseKd0
			self.Kp1 = Settings.reverseKp1
			self.Ki1 = Settings.reverseKi1
			self.Kd1 = Settings.reverseKd1
			self.Kp2 = Settings.reverseKp2
			self.Ki2 = Settings.reverseKi2
			self.Kd2 = Settings.reverseKd2
			self.clipMax = Settings.reverseClipI
			self.clipMin = -Settings.reverseClipI
	...
	def RunLoop(self, (line0, local0, d1, d2)):
		with self.lock:
			...
			self.d0 = self.Kd0 * (d0 - self.lastD0)
			...
			self.d1 = self.Kd1 * (d1 - self.lastD1)
			...
			self.d2 = self.Kd2 * (d2 - self.lastD2)
	...
	def ProcessImage(self, image):
		...
		if Globals.imageMode == READY_TO_RACE:
			...
			if Globals.frontRobot:
				...
				# We get our start lights information from the front robot
				# Wait until we hear the GO signal
				if Globals.startLights == THIRD_GREEN_GO:
					# Ready, start racing
					Globals.MonsterLed(0, 1, 1)
					LogData(LOG_CRITICAL, 'Lights: GO')
					if Settings.firstStraightOverride:
						SetImageMode(FIRST_STRAIGHT)
					else:
		...
		if Globals.frontRobot:
			...
			# We get our start marker information from the front robot
			# See DataTransmission - NewDataRear
			pass
		...
		for Y in Settings.croppedYScan:
			...
			if Globals.frontRobot:
				...
				# Reverse direction
				self.FindMatches(Y, rFallingX, bRisingX,  kRisingX,  gRisingX, others, brLine, krLine, wrongWay)
				self.FindMatches(Y, rRisingX,  bFallingX, gFallingX, None,     others, rbLine, rgLine, None)
				self.FindMatches(Y, gRisingX,  bFallingX, kFallingX, None,     others, gbLine, gkLine, None)
				self.FindMatches(Y, bRisingX,  gFallingX, None,      None,     others, bgLine, None,   None)
